...
class $modify(FiveFivePlayLayer, PlayLayer) {
	...
	void destroyPlayer(PlayerObject* player, GameObject* cause) {
		...
		if (Mod::get()->getSettingValue<bool>("onlyondeath")) {
			...
			if (percent >= 92 && percent < 93) {
				this->scheduleOnce(schedule_selector(FiveFivePlayLayer::setUpForVortrox), 0.3f);
			}
		}
		PlayLayer::destroyPlayer(player, cause);
	...
	void setUpForVortrox(float dt) {
		if (Mod::get()->getSettingValue<bool>("vortrox") == false) return;
		pauseGame(false);
		...
		auto graphicSetup = CCSprite::create("phobos.webp"_spr);
		auto graphic = imgp::AnimatedSprite::from(graphicSetup);
		graphic->setForceLoop(false);
		graphic->setCurrentFrame(0);

		if (!OverlayManager::get()->getChildByID("vortroxAnim"_spr)) {
			OverlayManager::get()->addChild(graphic);
			auto winSize = CCDirector::get()->getWinSize();
			graphic->setPosition(winSize / 2);
			graphic->setID("vortroxAnim"_spr);
			graphic->play();
			if (Mod::get()->getSettingValue<bool>("fullscreen")) {
				auto contentSize = graphic->getContentSize();
				graphic->setScaleX(winSize.width / contentSize.width);
...
class $modify(FiveFivePauseLayer, PauseLayer) {
    struct Fields {
		...
		~Fields() {
			...
			if (auto vortrox = OverlayManager::get()->getChildByID("vortroxAnim"_spr)) {
				vortrox->removeFromParent();
			}
		...
		Loader::get()->queueInMainThread([this]() {
			if (auto scene = CCScene::get()) {
				if (auto pauseLayer = scene->getChildByType<PauseLayer>(0)) {
					if (OverlayManager::get()->getChildByID("fiveFiveAnim"_spr) || OverlayManager::get()->getChildByID("vortroxAnim"_spr)) {
						this->schedule(schedule_selector(FiveFivePauseLayer::checkIfDone));
						m_fields->canUnpause = false;
						
						std::filesystem::path audioFile;
						float percent = PlayLayer::get()->getCurrentPercent();

						if (Mod::get()->getSettingValue<bool>("vortrox") && (percent >= 92 && percent < 93)) {
							audioFile = Mod::get()->getResourcesDir() / "phobos.mp3";
						} else {
							audioFile = Mod::get()->getResourcesDir() / "fivefive.mp3";
						}
	...
	void checkIfDone(float dt) {
		imgp::AnimatedSprite* graphic = nullptr;

		if (Mod::get()->getSettingValue<bool>("vortrox")) {
			graphic = static_cast<imgp::AnimatedSprite*>(OverlayManager::get()->getChildByID("vortroxAnim"_spr));
		}
	
		if (!graphic) {
			graphic = static_cast<imgp::AnimatedSprite*>(OverlayManager::get()->getChildByID("fiveFiveAnim"_spr));
			if (!graphic) return;
		}
	...
	void canUnpause(float dt) {
		...
		if (Mod::get()->getSettingValue<bool>("mobilebutton")) return;
		addResumeButton();
		return;
	}
